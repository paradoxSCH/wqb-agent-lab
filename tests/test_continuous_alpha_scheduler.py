...
class ContinuousAlphaSchedulerTests(unittest.TestCase):
    ...
    def test_constructor_canonicalizes_workspace_and_state_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            scheduler = ContinuousAlphaScheduler(root / "unused" / "..", state_path)

            self.assertEqual(root, scheduler.workspace_root)
            self.assertEqual(state_path, scheduler.state_path)
