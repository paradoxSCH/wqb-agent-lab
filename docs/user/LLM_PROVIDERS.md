...
旧版规划输出仍是默认值。用户可以显式启用 provider-neutral proposal envelope 和有界结构修复：
...
修复只处理 JSON 和契约结构。未知机制、任意表达式、未来 action kind、alternatives、
extension data 和 free-form notes 仍是有效研究内容。Proposal 只能请求动作，不能启用
live simulation 或 submission capability。
