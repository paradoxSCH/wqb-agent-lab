...
- `runtime`：Python 研究引擎、WQB 平台边界、研究政策、记忆与本地演示。推荐所有用户从这里开始，不需要 Node.js。
...
本地演示通过后，再按 [研究政策](RESEARCH_POLICY.md) 和 [LLM Provider](LLM_PROVIDERS.md) 配置。WQB 邮箱、密码和模型密钥只能写入 `.env`。首次安装不得把以下 capability 改为 `1`：
